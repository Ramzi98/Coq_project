...
Proof. simpl. reflexivity.  Qed.
Example test_ltb4:             (ltb 3 2) = false.
Proof. simpl. reflexivity.  Qed.
Example test_ltb5:             (ltb 2 3) = true.
Proof. simpl. reflexivity.  Qed.
(** [] *)
...
Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof. intros n m o.  (*On a 2 hypothese donc on a besoin de 2 hypothese intros et 2 rewrite*) 
       intros H H'.
       rewrite -> H.
       simpl.
       rewrite -> H'. 
...
Fixpoint bin_to_nat (m:bin) : nat :=
  ...
  | B m' => plus 1 (mult' 2 (bin_to_nat m'))
end.
...
Example test1 : bin_to_nat(Z) = 0.
Proof. simpl. reflexivity. Qed.

Example test2 : bin_to_nat(B(Z)) = 1.
Proof. simpl. reflexivity. Qed.

Example test3 : bin_to_nat(incr (B(Z))) = 2.
Proof. simpl. reflexivity. Qed.

Example test4 : bin_to_nat (A( B (B Z))) = 6.
Proof. simpl. reflexivity. Qed.

Example test5 : bin_to_nat( incr  (A( B (B Z)))) = 7.
Proof. simpl. reflexivity. Qed.

Example test6 : bin_to_nat((B(A (A (B Z))))) = 9.
Proof. simpl. reflexivity. Qed.

Example test7 : bin_to_nat((A(B (A (B Z))))) = 10.
Proof. simpl. reflexivity. Qed.

Example test8 : bin_to_nat((B(B (B (B Z))))) = 15.
Proof. simpl. reflexivity. Qed.

Example test9 : bin_to_nat(B(B(A(B Z)))) = 11.
Proof. simpl. reflexivity. Qed.
