...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. simpl. induction l.
  -reflexivity. 
  -simpl. rewrite->IHl. reflexivity.
Qed.
...
(** **** Exercise: 2 stars, standard, recommended (split)  
    ...
    given unit test. *)
Definition append_list_pair {X Y : Type} (lp1 : (list X) * (list Y)) (lp2 : (list X) * (list Y)) 
    : (list X) * (list Y) :=
...
(*Fonction countoddmemebrs countig the numbre of impair numbre in the lists*) 
...
Proof.
  intros X Y l f. induction f.
  -simpl. reflexivity.
  -simpl. rewrite<-IHf. simpl. rewrite->map_assoc. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros X Y Z f. intros x y. simpl. reflexivity.
Qed.
...
Proof.
  intros X Y Z f. intros p. induction p.
  -simpl. reflexivity.
Qed. 
(** [] *)
...
(* Difference entre explicite et implicite 
    implicite n'est pas formelment exprimé ,
    explicite exprimé dans la diffintion de la fonction*)
(* c'est quoi une liste polymorph = c'est une listes de type ginérique ca veut dire 
    une listes de nat ou bool .... peut etre n'importe quel type en utilise list poly 
    pour ne pas definir a chaque fois une listes comme nat-list bool-list 
    en difiner list polymorph et son type definier soit lors de l'utilisation ou bien 
    implicitement paraport le type des paramateres utiliser *)
